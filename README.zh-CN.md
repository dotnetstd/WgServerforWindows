...
# WS4W v2.3.0 - 更新内容
* **开机自启与后台运行**：新增开机自动启动功能，并支持配合系统托盘图标实现后台运行（--minimized 参数）。
* **UI 交互优化**：移除了传统的启动页，引入了加载遮罩层和 IsBusy 状态管理；标题栏现已显示软件版本号。
* **窗体记忆**：现在软件可以记住上次关闭时的窗口大小和位置（排除最小化和最大化）。
* **参数持久化修复**：修复了 IPv6 同步参数在设定后无法保存到本地的问题。
* **多语言支持**：完善了新功能（开机自启、MTU 向导、日志中心、动态 IP）的翻译。
* **稳定性提升**：修复了“设定”页面可能导致的崩溃问题。
